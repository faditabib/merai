...
## Corpus Check
- 210 files · ~131,186 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1534 nodes · 2534 edges · 146 communities (88 shown, 58 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 3 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `acfb46de`
...
1. `Merai — Architectural Decisions` - 41 edges
2. `TranscriptWord` - 38 edges
3. `EdlV1` - 29 edges
4. `Merai — Progress Log` - 25 edges
...
- `CaptionOverlayProps` --references--> `CaptionBrandColors`  [EXTRACTED]
...
## Communities (146 total, 58 thin omitted)
...
Cohesion: 0.12
Nodes (20): images, line, PermanentJobError, defaultDeps, OutputTooLargeError, renderExport(), RenderExportDeps, renderExportWithEngine() (+12 more)

### Community 5 - "AI Analysis & EDL Builder"
Cohesion: 0.08
Nodes (35): ANALYSIS_TOOL, HaikuAnalysisEngine, MessageCreator, renderTranscript(), HeuristicAnalysisEngine, createAnalysisEngine(), AnalysisEngine, AnalysisInput (+27 more)
...
Cohesion: 0.11
Nodes (33): Phase, RecordFlow(), SetupError, Take, attachVideo(), clampPipWidth(), compositeFrameSize(), CompositeHandle (+25 more)
...
Cohesion: 0.11
Nodes (17): Build 5.5 — AI Editing Brain v1 (2026-07-11), Build 5.6 — AI Brain UX polish + feedback loop (2026-07-11), Build 5 — Professional editing core: EDL v2 foundation (2026-07-11), Build 6A.1 — Visual QA fixes + production redeploy (2026-07-11), Build 6A — Creator experience layer (2026-07-11), Build 6B.1 — Creator Identity Layer (2026-07-11), Build 6B.2 — Caption Studio (2026-07-12), Build 6B.3 — Caption Studio UX (2026-07-12) (+9 more)
...
Cohesion: 0.05
Nodes (41): 2026-07-08 — 10-minute cap enforced in three layers, no ffprobe infra, 2026-07-08 — Arabic language handling: keep auto-detection, no pin, 2026-07-08 — Auth: Supabase email+password to start, 2026-07-08 — Brand terms: AssemblyAI custom_spelling adopted, word_boost rejected, 2026-07-08 — Browser locale detection disabled, 2026-07-08 — DB-level tests run on PGlite applying the real migrations, 2026-07-08 — EDL is versioned, immutable, append-only JSON (v1 single-track), 2026-07-08 — Enum-like columns are text + CHECK constraints, not Postgres enums (+33 more)
...
Cohesion: 0.06
Nodes (39): AiAssistantPanel(), FEEDBACK_REASONS, INTENTS, PRESET_KEYS, SuggestionRow, createEditBrain(), EditBrain, HaikuEditBrain (+31 more)
...
Cohesion: 0.18
Nodes (10): BrandKitPage(), OnboardingPage(), RecordPage(), AppHeader(), BrandKitForm(), LocaleSwitcher(), SignOutButton(), { Link, redirect, usePathname, useRouter, getPathname } (+2 more)

### Community 96 - "project-status-view.tsx"
Cohesion: 0.17
Nodes (20): CaptionPreview(), CaptionPreviewProps, captionSpanStyle(), anchorToPosition(), CaptionStudio(), CaptionStudioProps, DEFAULT_OUTLINE, Position (+12 more)
...
Cohesion: 0.29
Nodes (10): EditorView(), ShortcutsHelp(), ReorderDrag, Timeline(), TrimDrag, applyEditCommands(), edlOutputDurationMs(), nextSegmentAfterSource() (+2 more)
...
Cohesion: 0.22
Nodes (14): CaptionOverlay(), activeCaptionIndex(), activeWordIndex(), buildCaptionLines(), CAPTION_PRESET_IDS, CAPTION_PRESETS, CAPTION_STYLE_TOKENS, CaptionAnimation (+6 more)
...
Cohesion: 0.19
Nodes (19): applyEditCommand(), aspectRatioSchema, edlV1Schema, KeptSegment, keptSegmentSchema, nextSegmentId(), outputToSourceMs(), removeWords() (+11 more)

### Community 107 - "caption-studio.tsx"
Cohesion: 0.33
Nodes (7): renderBrandImages(), renderGradientImage(), renderLowerThirdImage(), rgba(), registerCaptionFonts(), LOGO_CFG, LogoOverlayConfig
...
Cohesion: 0.06
Nodes (51): BrandKitFormProps, GRADIENT_DEFAULTS, LOGO_TYPES, CreatorStylePreview(), CreatorStylePreviewProps, CreatorStyles(), CreatorStylesProps, LOGO_TYPES (+43 more)
...
Cohesion: 0.05
Nodes (38): 1. Current state (what 7.1 plugs into), 2.1 Recorder core (`apps/web/src/lib/record/recorder.ts` — pure + thin wrapper), 2.2 Record page (`/dashboard/record`, `RecordFlow` client component), 2.3 Entry point, 2. Design, 3. Database / worker impact, 4. Files to touch, 5. Risks & mitigations (+30 more)
...
Cohesion: 0.13
Nodes (15): cleanupExpired(), handlers, JobHandler, AnalyzePayload, analyzePayloadSchema, cleanupExpiredPayloadSchema, GenerateEdlPayload, generateEdlPayloadSchema (+7 more)

### Community 132 - "transcribe.ts"
Cohesion: 0.30
Nodes (9): completeUpload(), CreateUploadResult, requestAiEdit(), requestExportRender(), retryProcessing(), GET(), NewProjectPage(), createAdminClient() (+1 more)
...
## Knowledge Gaps
- **757 isolated node(s):** `eslintConfig`, `withNextIntl`, `nextConfig`, `name`, `version` (+752 more)
...
- **Why does `EdlV1` connect `EdlV1` to `project-status-view.tsx`, `editor-view.tsx`, `edl-v2.ts`, `Server Render Pipeline`, `AI Analysis & EDL Builder`, `TranscriptWord`, `edl-ops.ts`, `export-plan.ts`, `fillers.ts`, `brand-kit-form.tsx`, `RenderRequest`?**
  _High betweenness centrality (0.009) - this node is a cross-community bridge._
- **Why does `TestDb` connect `upload-flow.tsx` to `RenderRequest`, `Server Render Pipeline`, `render-export.test.ts`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `TranscriptWord` connect `EdlV1` to `project-status-view.tsx`, `editor-view.tsx`, `edl-v2.ts`, `AI Analysis & EDL Builder`, `Transcription Providers`, `TranscriptWord`, `edl-ops.ts`, `export-plan.ts`, `upload-flow.tsx`, `fillers.ts`, `brand-kit-form.tsx`, `RenderRequest`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
